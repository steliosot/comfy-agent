---
name: generate_sd15_cinematic_portrait
description: >
  Generate a cinematic SD1.5 portrait image.
...
# generate_sd15_cinematic_portrait
...
```bash
python3 skills/workflows/generate_sd15_cinematic_portrait/scripts/run.py --args '{}' --pretty
```

Pass JSON kwargs for `run(...)` via `--args`:

```bash
python3 skills/workflows/generate_sd15_cinematic_portrait/scripts/run.py --args '{"param":"value"}' --pretty
```

## Inputs / Outputs

- Inputs follow `skills/workflows/generate_sd15_cinematic_portrait/skill.yaml`.
