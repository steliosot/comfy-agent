---
name: generate_sd15_stelios_shoe_ad
description: >
  Generate an SD1.5 animated shoe advertisement for Stelios.
...
```bash
python3 skills/workflows/generate_sd15_stelios_shoe_ad/scripts/run.py --args '{}' --pretty
```

Pass JSON kwargs for `run(...)` via `--args`:

```bash
python3 skills/workflows/generate_sd15_stelios_shoe_ad/scripts/run.py --args '{"param":"value"}' --pretty
```

## Inputs / Outputs

- Inputs follow `skills/workflows/generate_sd15_stelios_shoe_ad/skill.yaml`.
