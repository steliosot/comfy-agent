...
```bash
python3 skills/workflows/preview_sd15_fast_character/scripts/run.py --args '{}' --pretty
```

Pass JSON kwargs for `run(...)` via `--args`:

```bash
python3 skills/workflows/preview_sd15_fast_character/scripts/run.py --args '{"param":"value"}' --pretty
```

## Inputs / Outputs

- Inputs follow `skills/workflows/preview_sd15_fast_character/skill.yaml`.
