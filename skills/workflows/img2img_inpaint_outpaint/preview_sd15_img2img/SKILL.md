...
description: >
  Preview an SD1.5 img2img result from an input image.
...
```bash
python3 skills/workflows/preview_sd15_img2img/scripts/run.py --args '{}' --pretty
```

Pass JSON kwargs for `run(...)` via `--args`:

```bash
python3 skills/workflows/preview_sd15_img2img/scripts/run.py --args '{"param":"value"}' --pretty
```

## Inputs / Outputs

- Inputs follow `skills/workflows/preview_sd15_img2img/skill.yaml`.
