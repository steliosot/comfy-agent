...
```bash
python3 skills/workflows/generate_sd15_img2img_remix/scripts/run.py --args '{}' --pretty
```

Pass JSON kwargs for `run(...)` via `--args`:

```bash
python3 skills/workflows/generate_sd15_img2img_remix/scripts/run.py --args '{"param":"value"}' --pretty
```

## Inputs / Outputs

- Inputs follow `skills/workflows/generate_sd15_img2img_remix/skill.yaml`.
