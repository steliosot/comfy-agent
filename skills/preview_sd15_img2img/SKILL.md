...
```bash
python3 skills/preview_sd15_img2img/scripts/run.py --args '{}' --pretty
```

Pass JSON kwargs for `run(...)` via `--args`:

```bash
python3 skills/preview_sd15_img2img/scripts/run.py --args '{"param":"value"}' --pretty
```

## Inputs / Outputs

- Inputs follow `skills/preview_sd15_img2img/skill.yaml`.
