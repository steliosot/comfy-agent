...
```bash
python3 skills/generate_sd15_compat_combo/scripts/run.py --args '{}' --pretty
```

Pass JSON kwargs for `run(...)` via `--args`:

```bash
python3 skills/generate_sd15_compat_combo/scripts/run.py --args '{"param":"value"}' --pretty
```

## Inputs / Outputs

- Inputs follow `skills/generate_sd15_compat_combo/skill.yaml`.
